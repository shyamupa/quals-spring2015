...
\begin{frame}{Partition Function Woes}
  ...
  \item   ML parameter estimation,
    ...
    \end{align*}
  \item   Computing $\E_{\Pr(x;\theta)}$ is not usually tractable (Sampling from posterior is hard).
  \item Resort to approximations - MCMC, Contrastive Divergence etc.
  \end{itemize}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Wait ... So What?}
  All this is cute, but what is the benefit?
  \begin{itemize}
  \item No need to compute the partition function, as long as you can compute the (perturbed) argmax!
  \item Note that the samples are exact. (No approximations!)
