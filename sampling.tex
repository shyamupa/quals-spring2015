...
\begin{frame}{Introduction - Partition Function Woes}
  ...
  \item Gibbs Distribution
    ...
    \end{align*}
    %% standard/popular way of probabilistic modeling 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[label=framelabel]{Detour - Gumbel Distribution}
  %% mathematical machinery 
  \begin{itemize}[<+->]
  \item Models distribution of maximas of samples from exponential-like distributions.
  ...
  \item CDF for $Gumbel(m)$, $\Pr(G\le g) = \exp(-\exp(-g+m))$
    ...
    \begin{figure}
      \centering
      \includegraphics[scale=0.7]{images/gumbel.png}
    \end{figure}   
  \end{itemize}
\end{frame}
...
\begin{frame}{Gumbel-Max Trick (Discrete Case)}
  ...
  \onslide<2->
  Easy Sampling with un-normalized log probabilities,
  \begin{align}
  ...
  \begin{align*}
    ...
    %% \tag{the argmax of the perturbed probabilities is distributed as eq.\ref{eq:gibbs} above}
  \end{align*}
\end{frame}
