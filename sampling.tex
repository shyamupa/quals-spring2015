...
\begin{frame}{Top-Down Construction}
  ...
  \begin{center}
    ...
    \only<3>{
      $G_1 \sim Gumbel(\log\mu(\Omega))$ \\
      \begin{center} 
      assume $\log\mu(\Omega)$ is computable for now.
      \end{center}
...
\begin{frame}<1>[label=progress]{Progress So Far}
  ...
  \begin{overprint}
    ...
    \textbf{Problems}
    \begin{itemize}
    \item {\color{red}Problem 1:} Computing $\int \exp(\phi(x))$        % Drawing samples from the distribution requires knowing Z of my target distribution.
...
\begin{center}
  ...
  \only<5>{
    Not as simple, as sometimes we only know upper and lower bounds of $o(X)$.
  }
\end{center}
\end{frame}
