...
\begin{frame}{Generating Vector Representations}
  ...
  \item Cooccurence Matrix Factorization ~\cite{Deerwester}
    % HAL,LSA the name of the game is compute co-occurence
...
\begin{frame}{GloVe Model - Intuition}
  ...
  %% whereas in solid and gas the affinity shifts towards a particular word
  \visible<2>{
      \begin{align*}
...
\begin{frame}{GloVe Model - Derivation of Objective}
  ...
  \begin{center}
    \visible<6->{But wait!}
  \end{center}
%% The problem is that X_ij are estimated from data. Some values will be very large/small, so our vectors will devote more/less time reducing the loss on them.
\end{frame}
...
\begin{frame}{GloVe Model - Introduce Weighted Cost}
  ...
  \visible<2->{
    \begin{align*}
      ...
      %% x_max prevents too-freq t overweigh
